...
6 维度为并列独立拆解维度；不同维度存在重复归因，不跨维度简单加总。旧口径里的「品类」「FAQ」「战区」等模糊命名仅作为兼容别名，输出与报告展示统一到上表标准字段。
...
HTML 与 Markdown 报告共用这 5 个内容模块；视觉组件数量（指标卡片、进度条、趋势图、明细表等）不作为报告模块计数口径。

### 事实分层声明

本项目为求职作品集演示，使用模拟数据。文档中的效率提升、准确率、满意度、用户量、报告量、成本等数字，如无真实数据支撑，均标注为「复盘测算」「目标值」或「示例值」，不写成真实线上成果。
...
归因排序使用“万服波动贡献”，服务量变化占比只作为辅助字段：

```text
万服波动贡献 = 维度项本期服务量 / 本期整体订单量 * 10000
             - 维度项对比期服务量 / 对比期整体订单量 * 10000

服务量变化占比 = 某因素服务量变化 / 本期整体服务量
```

API 字段 `wanfu_contribution` 表示万服波动贡献，用于 Top 推高/压低排序；`service_change_ratio` 表示服务量变化占比，仅用于说明服务量规模变化。旧字段 `contrib_wanfu` 暂时保留用于兼容旧前端或旧接口，并跟随 `wanfu_contribution`。
...
- 服务量：92568
- 订单量：7684020
- Top 推高因素：华南战区，按万服波动贡献排序
- Top 压低因素：退款进度，按万服波动贡献排序
